...
export class AuthController {
  ...
  static async otpLogin(req: Request, res: Response, next: NextFunction) {
    try {
      const { phone, otp, name } = req.body;

      if (otp !== '123456') {
        return res.status(400).json({ success: false, message: 'Invalid 6-digit OTP code. Hint: Use 123456' });
      }

      // Live Mode
      let user = await User.findOne({ phone });
      if (!user) {
        const mockEmail = `farmer_${phone}@krishimitra.com`;
        const mockPassword = Math.random().toString(36).substring(2, 15);
        user = await User.create({
