...
export const AdminProviders: React.FC = () => {
  ...
  const handleToggleStatus = async (provider: ProviderItem, enableTarget: boolean) => {
    ...
    try {
      ...
      setProviders(prev => prev.map(p => p.id === provider.id ? { ...p, enabled: enableTarget, status: enableTarget ? 'ACTIVE' : 'INACTIVE' } : p));
      
      const res = await api.patch(`/admin/providers/${provider.id}/status`, { enabled: enableTarget });
      const msg = res.data?.message || `Provider ${enableTarget ? 'enabled' : 'disabled'} successfully.`;
      
      showToast(msg, enableTarget ? 'success' : 'info');
      setConfirmDisable(null);
      await fetchProviders(true);
    } catch (err: any) {
      // Revert optimistic update on failure
      setProviders(prev => prev.map(p => p.id === provider.id ? { ...p, enabled: !enableTarget, status: !enableTarget ? 'ACTIVE' : 'INACTIVE' } : p));
      showToast(`Failed to update status for ${provider.name}: ${err.response?.data?.message || err.message}`, 'error');
    } finally {
      setTogglingId(null);
    }
  };
                ...
                <div className="space-y-3 pt-3 border-t border-gray-100 dark:border-dark-800">
                  ...
                  <div className="flex items-center justify-between gap-1 text-[11px]">
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 text-[11px] font-extrabold rounded-lg transition-all flex items-center gap-1 disabled:opacity-50 ${
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-dark-900 border border-gray-100 dark:border-dark-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="pt-3 flex justify-end gap-2">
              <button
                disabled={togglingId === confirmDisable.id}
              ...
              <button
                disabled={togglingId === confirmDisable.id}
                onClick={() => handleToggleStatus(confirmDisable, false)}
                className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl disabled:opacity-50 flex items-center gap-2"
              >
